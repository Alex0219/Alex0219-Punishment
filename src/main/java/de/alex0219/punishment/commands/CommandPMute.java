            ...
            if (!(args.length < 2)) {
                ...
                if (PunishmentBootstrap.getInstance().getRankManager().isPermittedToBan(executor, bannedPlayer)) {
                    if (PunishmentBootstrap.getInstance().getBanManager().isBanned(bannedPlayer)) {
                        commandSender.sendMessage(new TextComponent("§bAlex0219.de §7» §cDieser Spieler ist bereits gemutet!"));
                        return;
                    }
                    PunishmentBootstrap.getInstance().getBanManager().mutePlayerPermanently(executor, bannedPlayer, reason);
                    commandSender.sendMessage(new TextComponent("§bAlex0219.de §7» Der Spieler §a" + bannedPlayer.getName() + " §7wurde erfolgreich §7gemutet."));
                } else {
                    commandSender.sendMessage(new TextComponent("§bAlex0219.de §7» §cDu darfst diesen Spieler nicht muten!"));
                    return;
                }
            } else {
                commandSender.sendMessage(new TextComponent("§bAlex0219.de §7» Bitte verwende /pmute <Spieler> <Grund>"));
            }
        } else {
